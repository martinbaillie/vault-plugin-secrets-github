...
func (c *Client) Token(ctx context.Context, opts *tokenOptions, organization string) (*logical.Response, error) {
	if organization != "" && c.InsID == 0 {
		instID, err := c.GetInstallationID(ctx, organization)
		if err != nil {
			return nil, err
		}
...
func (c *Client) GetInstallationID(ctx context.Context, organization string) (*string, error) {
	...
	for _, v := range instResult {
		if v.Account.Login == organization {
			instID = v.ID
			break
		}
	}
