...
5. Configure the plugin with the details from the previous section.
   ...
   #+BEGIN_SRC shell
    ...
    vault write /github/config app_id=<app_id> org_name=<organization_name> prv_key=@<private_key_file>
...
vault write /github/config app_id=123 org_name="bla-bla" prv_key=@key.pem
