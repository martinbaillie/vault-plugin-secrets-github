...
** Permission set
Instruct the plugin to create an permission set.
...
| GET    | /permissionsets/      | application/json |
...
Instruct the plugin to create an installation access token against the configured GitHub App from the given permission set.
